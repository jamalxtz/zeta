 ...
 *
 *esse login é bem estranho ele faz uma verificação se os dados estão chegando pela session ou pelo post
 *em seguida ele verifica se o usuario existe e depois verifica a senha
 *
 * @package TutsupMVC
 * @since 0.1
 */
class UserLogin
{
	...
	public function check_userlogin () {
		...
		if ( ! $query ) {
			$this->logged_in = false;
			$this->login_error = 'Erro ao consultar o BD.';
		...
		if($fetch['situacao'] == "Inativo"){
			$this->logged_in = false;
			$this->login_error = 'Usuário inativo no sistema!';
		...
		if ( $this->phpass->CheckPassword( $user_password, $fetch['senha'] ) ) {
			...
			if ( session_id() != $fetch['user_session_id'] && ! $post ) { 
				$this->logged_in = false;
				$this->login_error = 'Erro no ID da session.';
				// Desconfigura qualquer sessão que possa existir sobre o usuário
				$this->logout();
